            ...
            <?php foreach($seized_vechicles as $key=>$data):?>
                                    <tr style="border-bottom:1px solid lightgray;">
                                        <td><?php echo $data->formserialno; ?></td>
                                          ...
                                          <td><?php if($data->clearnotclearstatus == 1){
										  echo "<p style='color:green;'>Cleared</p>";}
										   elseif($data->clearnotclearstatus == 0){ echo
										   "<p style='color:yellow;'>pending</p>";}
										  else{ echo "<p style='color:Red;'>Not Cleared</p>";}?></td>



                                          <?php
												 if(empty($data->mracategories)){
											  $cat1 =0;
										  }else{
											  $cat1 = $data->mracategories;

										  }

										  //$vehicleseize_category = explode(",",$cat1);
										  //print_r($vehicleseize_category);?>
											   <td>
										<?php
										 $query = custom_query("select * from tbl_vehicle_seized_categories where
										FIND_IN_SET(siezedid,'$cat1')","result"); ?>
															<?php  if(empty($query)){ echo "None"; }
															else{
										 ...
										 <?php endforeach;  }
										  ...
										  <?php if(empty($data->categories)){
											  ...
											  $cat1 = $data->categories;
										  }

										  //print_r($vehicleseize_category);

										  ?>
										  <td><?php if($data->fslclearnotclear == 1) {echo
										  "<span style='color:green;'>Cleared
										  </span>";}
											 elseif($data->fslclearnotclear == 0){
												 echo "<p style='color:yellow;'>pending</p>";}
											else{
											echo "<span style='color:Red;'>Not Cleared</span>";
											}?></td>
											   <td>
										 <?php
										 $query = custom_query("select * from tbl_vehicle_seized_categories
										 where
										 FIND_IN_SET(siezedid,'$cat1')","result");
										 ?>
															<?php if(empty($query)){ echo "None"; }else{
																foreach($query as $category):

												 echo $category->seizedtype."</br>";
                                         ...
                                         <td><a href="<?php echo
										 site_url("Vehicles/Both_mra_fsl_details/{$data->vechileid}"); ?>"
									...
									<div class="btn-group" role="group">
										...
										<div class="btn-group" role="group">
											...
											<ul class="dropdown-menu">
												...
												<li><a href="javascript:void(0);" class="waves-effect waves-block" id="show_val<?php echo $data->vechileid; ?>"  onclick="clicked_v2(this);" custom="<?php echo $data->vechileid; ?>">Confiscat</a></li>
